...
class COCOeval:
    ...
    def summarize(self, per_category=False, final_pr=False):
        ...
        def _summarizeDets(kind=-1):
            ...
            stats[1] = _summarize(1, iouThr=.5, maxDets=self.params.maxDets[2], kind=kind, final_pr=final_pr)
            stats[2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2], kind=kind, final_pr=final_pr)
            stats[3] = _summarize(1, areaRng='small', maxDets=self.params.maxDets[2],kind=kind, final_pr=final_pr)
            stats[4] = _summarize(1, areaRng='medium', maxDets=self.params.maxDets[2], kind=kind, final_pr=final_pr)
            stats[5] = _summarize(1, areaRng='large', maxDets=self.params.maxDets[2], kind=kind, final_pr=final_pr)
            if not final_pr:
                ...
                stats[9] = _summarize(0, areaRng='small', maxDets=self.params.maxDets[2], kind=kind)
                stats[10] = _summarize(0, areaRng='medium', maxDets=self.params.maxDets[2], kind=kind)
                stats[11] = _summarize(0, areaRng='large', maxDets=self.params.maxDets[2], kind=kind)
            return stats
        def _summarizeKps():
            stats = np.zeros((10,))
        ...
        if per_category and summarize == _summarizeDets:
            ...
            for i, catId in enumerate(self.params.catIds):
                print("\n")
                print("######## Eval category: {}, id = {} ########".format(self.cocoGt.getCatName(catId), catId))
                summarize(i)
